...
def train():
    ...
    for epoch in range(n_epoch):
        ...
        while i < len_dataloader+1:
            ...
            domain_label_src = torch.zeros(batch_size).float().to(device)
            err_s_domain = loss_domain(domain_output, domain_label_src)
            ...
            domain_label_tag = torch.ones(batch_size).float().to(device)
            err_t_domain = loss_domain(domain_output, domain_label_tag)

            err = err_s_label + (err_s_domain + err_t_domain)*0.1
            err.backward()
            optimizerG.step()
        ...
        if epoch % 1 == 0:
            ...
            disp_str = 'Epoch:[%d/%d], source_acc:%f, val_acc:%f' % (epoch, n_epoch, acct, accu)
            print(disp_str)
        ...
        if epoch % 1 == 0:
            ...
            disp_str = 'Epoch:[%d/%d], source_val_acc:%f, target_acc:%f' % (epoch, n_epoch, accu, acctrg)
            print(disp_str)
            targetLogger.write(disp_str + '\n')
            targetLogger.flush()
            if current_tar_max < acctrg:
                current_tar_max = acctrg
            print('Current Alpha:', print_alpha, '  Current tar_max:', current_tar_max)
    disp_str = 'max_epoch:%d, max_val_acc:%f' % (max_epoch, max_accu)
    validLogger.write(disp_str + '\n')
    validLogger.flush()
