...
class DANN_USPSandMNIST(nn.Module):
    def __init__(self):
        super(DANN_USPSandMNIST, self).__init__()
...
def load_model(category):
    if category == 'utom' or category == 'mtou':
        return DANN_USPSandMNIST()
    elif category == 'stom':
        return DANN_SVHN()


def load_DANN(num_class=31):
    netG = DANN(num_class)
    url = 'https://download.pytorch.org/models/resnet50-19c8e357.pth'

    # pretrained_dict = torch.load('./dataset/resnet50-19c8e357.pth')
