...
def train():
    ...
    for epoch in range(1, n_epoch+1):
        ...
        print('Learning rate:', LEARNING_RATE, '  Alpha:', alpha)
        if isinstance(netG, nn.DataParallel):
            optimizerG = optim.SGD([
                ...
                {'params': netG.module.domain_classifier.parameters(), 'lr': LEARNING_RATE},
            ], lr=LEARNING_RATE / 10, momentum=momentum, weight_decay=l2_decay)
        else:
            optimizerG = optim.SGD([
                {'params': netG.feature.parameters()},
                {'params': netG.class_classifier.parameters(), 'lr': LEARNING_RATE},
                {'params': netG.domain_classifier.parameters(), 'lr': LEARNING_RATE},
            ], lr=LEARNING_RATE / 10, momentum=momentum, weight_decay=l2_decay)
