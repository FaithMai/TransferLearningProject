...
@click.option('--img_size', type=int, default=160, help='input image size')
@click.option('--confidence_thresh', type=float, default=0.9, help='augmentation var loss confidence threshold')
@click.option('--teacher_alpha', type=float, default=0.99, help='Teacher EMA alpha (decay)')
@click.option('--unsup_weight', type=float, default=10.0, help='unsupervised loss weight')
@click.option('--cls_balance', type=float, default=0.01, help='Weight of class balancing component of unsupervised '
                                                              'loss')
@click.option('--cls_balance_loss', type=click.Choice(['bce', 'log', 'bug']), default='bug',
              help='Class balancing loss function')
@click.option('--learning_rate', type=float, default=1e-5, help='learning rate (Adam)')
@click.option('--pretrained_lr_factor', type=float, default=0.1,
              help='learning rate scale factor for pre-trained layers')
@click.option('--fix_layers', type=str, default='conv1,bn1,layer1',
              help='List of layers to fix')
@click.option('--double_softmax', is_flag=True, default=True)
@click.option('--use_dropout', is_flag=True, default=True)
@click.option('--src_scale_u_range', type=str, default='0.75:1.333',
...
@click.option('--src_affine_std', type=float, default=0.0, help='aug xform: random affine transform std-dev')
...
@click.option('--src_hflip', default=True, is_flag=True, help='aug xform: enable random horizontal flips')
@click.option('--src_intens_scale_range', type=str, default='0.75:1.333',
...
@click.option('--src_greyscale', is_flag=True, default=True,
...
@click.option('--tgt_scale_u_range', type=str, default='0.75:1.333',
              help='aug xform: scale uniform range; lower:upper')
@click.option('--tgt_scale_x_range', type=str, default='',
              help='aug xform: scale x range; lower:upper')
@click.option('--tgt_scale_y_range', type=str, default='',
              help='aug xform: scale y range; lower:upper')
@click.option('--tgt_affine_std', type=float, default=0.0, help='aug xform: random affine transform std-dev')
...
@click.option('--tgt_hflip', default=True, is_flag=True, help='aug xform: enable random horizontal flips')
@click.option('--tgt_intens_scale_range', type=str, default='0.75:1.333',
              help='aug colour; intensity scale range `low:high` (-1.5:1.5 for mnist-svhn)')
@click.option('--tgt_colour_rot_std', type=float, default=0.05,
              help='aug colour; colour rotation standard deviation')
@click.option('--tgt_colour_off_std', type=float, default=0.1,
              help='aug colour; colour offset standard deviation')
@click.option('--tgt_greyscale', is_flag=True, default=True,
              help='aug colour; enable greyscale')
...
@click.option('--img_pad_width', type=int, default=16,
              help='random cropping; padding width')
@click.option('--num_epochs', type=int, default=50, help='number of epochs')
@click.option('--batch_size', type=int, default=56, help='mini-batch size')
@click.option('--epoch_size', type=str, default='28000',
    ...
    with torch.cuda.device(gpu_id):
        ...
        def f_pred_tgt_mult(X_sup):
            ...
            for aug_i in range(len(X_sup)):
                ...
                y_pred = F.softmax(teacher_net(X_var), dim=1).data.cpu().numpy()
